...
describe('Perfil', function () {
  ...
  it("Save perfil[0]", function (done) {
    chai.request(apiUrl)
      ...
      .end(function (err, res) {
        ...
        PerfilModuleData.perfil_module[0].id_perfil_modulo = res.body.data.id;
        done();
      });
  });
  ...
  it("Update perfilModulo", function (done) {
    ...
    chai.request(apiUrl)
      ...
      .set('Authorization', 'Bearer ' + Login.token)
      .end(function (err, res) {
        FileHelper.saveToFile(__dirname + '/data/perfilmodule.updated.id.' + perfilModulo.id_perfil_modulo + '.json', JSON.stringify(res));
        chai.expect(res.body).to.have.property('status');
  ...
  it("List By Id", function (done) {
    ...
    chai.request(apiUrl)
      ...
      .set('Authorization', 'Bearer ' + Login.token)
      .end(function (err, res) {
        FileHelper.saveToFile(__dirname + '/data/perfilmoduleById.' + id + '.json', JSON.stringify(res));
        chai.expect(res.body).to.have.property('status');
        chai.expect(res.body).to.have.property('data');
        chai.expect(res.body.status).to.equal("ok");
        done();
  ...
  it("List By Id Perfil", function (done) {
    ...
    chai.request(apiUrl)
      ...
      .set('Authorization', 'Bearer ' + Login.token)
      .end(function (err, res) {
        FileHelper.saveToFile(__dirname + '/data/perfilmoduleByIdPerfil.' + id + '.json', JSON.stringify(res.body));
        chai.expect(res.body).to.have.property('status');
        chai.expect(res.body).to.have.property('data');
        chai.expect(res.body.status).to.equal("ok");
        done();
  ...
  it("Delete By Id", function (done) {
    ...
    chai.request(apiUrl)
      ...
      .set('Authorization', 'Bearer ' + Login.token)
      .end(function (err, res) {
        FileHelper.saveToFile(__dirname + '/data/perfilmodule.delete.id.' + id + '.json', JSON.stringify(res));
        chai.expect(res.body).to.have.property('status');
        chai.expect(res.body.status).to.equal("ok");
        chai.expect(res.body).to.have.property('data');
        chai.expect(res.body.data).to.have.property('row_count');
